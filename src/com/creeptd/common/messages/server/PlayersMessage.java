...
public class PlayersMessage extends ServerMessage {
	private static final long serialVersionUID = -5604163815296022758L;

	private static final String REG_EXP_PLAYERS = "PLAYERS((\\sPLAYER\\s\"([^\"]+)\"\\s([0-9]+)\\s\"([^\"]+)\"\\s([0-9]+)\\s([0-9]+)\\s\"([^\"]+)\")*)";
    private static final String REG_EXP_PLAYER = "[\\s]*\"([^\"]+)\"\\s([0-9]+)\\s\"([^\"]+)\"\\s([0-9]+)\\s([0-9]+)\\s\"([^\"]+)\"[\\s]*";
    ...
    public class Player {
        ...
        Player(String message) {
            ...
            if (matcher.matches()) {
                ...
                this.language = matcher.group(6);
            }
        }

        /** Create player from details */
        Player(String operation, int id, String name, int points, int skill, String language) {
        ...
        public String toString() {
            return "PLAYER \""+this.operation+"\" "+this.id+" \""+MessageUtil.prepareToSend(this.name)+"\" "+this.points+" "+this.skill+" \""+MessageUtil.prepareToSend(this.language)+"\"";
        }
    }
     ...
     */
    public void addPlayer(String operation, int id, String name, int points, int skill, String language) {
        Player p = new Player(operation, id, name, points, skill, language);
        this.players.add(p);
    }
