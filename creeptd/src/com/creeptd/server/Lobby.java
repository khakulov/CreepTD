    ...
    public static boolean kickClient(Player player, Client adminClient, boolean banUser, boolean banUserAndMac) {
        ...
        if (kik == false) {
            ...
            if (kickedClient != null) {
                ...
                sendAll(new ServerChatMessage("Server", "<span style=\"color:red;\">" + kickedClient.getPlayerModel().getName() + " was kicked by <b>" + adminClient.getPlayerModel().getName() + "</b></span>"));
        ...
        if (banUser == true) {
            ...
            try {
                ...
                adminClient.send(new ServerChatMessage("Server", "<span style=\"color:red;\">" + player.getName() + " (Account) has been banned!</span>"));

                logger.debug("Block for User " + player.getName() + " / " + player.getMac() + " saved.");
            } catch (Throwable t) {
                logger.error("error while saving block for User " + player.getName() + " / " + player.getMac() + " ", t);
            }
        }
        if (banUserAndMac == true) {
            adminClient.send(new ServerChatMessage("Server", "<span style=\"color:red;\">" + player.getName() + " (Mac) has been blacklisted!</span>"));
            ...
            try {
                ...
                logger.debug("Block for MAC " + player.getName() + " / " + player.getMac() + " saved.");
            } catch (Throwable t) {
                logger.error("error while saving block for MAC " + player.getName() + " / " + player.getMac() + " ", t);
            }
        }
        return kik;
    }
    ...
    public static boolean unBanClient(Player player, Client adminClient) {
        ...
        try {
            ...
            if (blacklist != null) {
                ...
                adminClient.send(new ServerChatMessage("Server", "<span style=\"color:red;\">" + player.getName() + " (Mac) has been unbanned !</span>"));
                retrun = true;
            }
            entityTransaction.commit();
        } catch (Throwable t) {
            logger.error("error while remove block for User " + player.getName() + " / " + player.getMac() + " ", t);
        }
        ...
        try {
            ...
            entityManager.flush();
            entityTransaction.commit();
            adminClient.send(new ServerChatMessage("Server", "<span style=\"color:red;\">" + player.getName() + " (User) has been unbanned !</span>"));

            logger.debug("undBlock for user " + player.getName() + " / " + player.getMac() + " removed.");
        } catch (Throwable t) {
            logger.error("error while saving undBlock for User " + player.getName() + " / " + player.getMac() + " ", t);
        }
        return retrun;
    }
}
