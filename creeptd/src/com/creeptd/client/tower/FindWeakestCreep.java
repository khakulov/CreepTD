 ...
 * @author mi7hr4ndir
 */
public class FindWeakestCreep implements FindCreepStrategy {
     ...
     *            tower
     */
    public FindWeakestCreep(Tower t) {
        tower = t;
    }
    ...
    public Creep findCreep() {
        ...
        for (int i = 0; i < size; i++) {
            if (tower.getContext().getCreeps().get(i).isValidTarget()) {
                ...
                // squared distance
                float dist = dX * dX + dY * dY;
                if (dist < distanceMin && tower.getContext().getCreeps().get(i).getHealth() < health) {
                    health = tower.getContext().getCreeps().get(i).getHealth();
