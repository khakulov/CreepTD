 ...
 * @author mi7hr4ndir
 */
public class FindFastestCreep implements FindCreepStrategy {
     ...
     *            tower
     */
    public FindFastestCreep(Tower t) {
        tower = t;
    }
    ...
    public Creep findCreep() {
        ...
        for (int i = 0; i < size; i++) {
            if (tower.getContext().getCreeps().get(i).isValidTarget()) {
                ...
                // squared distance
                float dist = dX * dX + dY * dY;
                if (dist < distanceMin && tower.getContext().getCreeps().get(i).getSpeed() > speed) {
                    // shoot immune Creeps only if there are no others to shoot
                    if ((!tower.getContext().getCreeps().get(i).isSlowImmune()) || (found == null)) {
                        speed = tower.getContext().getCreeps().get(i).getSpeed();
