...
public class Creep15 extends AbstractCreep {
     ...
     */
    public Creep15(GameContext context, IConstants.Creeps t) {
        super(context, t);
...
//		g2.dispose();
    ...
    public void loadImage() {
        ...
        if (Cache.getInstance().hasCreepImg(IConstants.Creeps.creep15)) {
            setImage(Cache.getInstance().getCreepImg(IConstants.Creeps.creep15));
        } else {

            try {
                BufferedImage forCache;
                this.image = ImageIO.read(this.getClass().getClassLoader().getResourceAsStream(
                        IConstants.CREEPS_URL + "15.png"));
                forCache = this.image;
                Cache.getInstance().putCreepImg(IConstants.Creeps.creep15, forCache);
