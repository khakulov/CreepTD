...
public class Creep10 extends AbstractCreep {
     ...
     */
    public Creep10(GameContext context, IConstants.Creeps t) {
        super(context, t);
        setSlowImmune(true);
...
//		g2.dispose();
    ...
    public void loadImage() {
        ...
        if (Cache.getInstance().hasCreepImg(IConstants.Creeps.creep10)) {
            setImage(Cache.getInstance().getCreepImg(IConstants.Creeps.creep10));
        } else {

            try {
                BufferedImage forCache;
                this.image = ImageIO.read(this.getClass().getClassLoader().getResourceAsStream(
                        IConstants.CREEPS_URL + "10.png"));
                forCache = this.image;
                Cache.getInstance().putCreepImg(IConstants.Creeps.creep10, forCache);
