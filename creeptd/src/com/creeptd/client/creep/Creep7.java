...
//		
    ...
    public void loadImage() {
        ...
        if (Cache.getInstance().hasCreepImg(IConstants.Creeps.creep7)) {
            setImage(Cache.getInstance().getCreepImg(IConstants.Creeps.creep7));
        } else {

            try {
                BufferedImage forCache;
                this.image = ImageIO.read(this.getClass().getClassLoader().getResourceAsStream(
                        IConstants.CREEPS_URL + "7.png"));
                forCache = this.image;
                Cache.getInstance().putCreepImg(IConstants.Creeps.creep7, forCache);
