...
public class Creep7 extends AbstractCreep {
     ...
     */
    public Creep7(GameContext context, Constants.Creeps t) {
        super(context, t);
...
//		
    ...
    public void loadImage() {
        ...
        if (Cache.getInstance().hasCreepImg(Constants.Creeps.creep7)) {
            setImage(Cache.getInstance().getCreepImg(Constants.Creeps.creep7));
        } else {

            try {
                BufferedImage forCache;
                this.image = ImageIO.read(this.getClass().getClassLoader().getResourceAsStream(
                        Constants.CREEPS_URL + "7.png"));
                forCache = this.image;
                Cache.getInstance().putCreepImg(Constants.Creeps.creep7, forCache);
